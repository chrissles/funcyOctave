...
%            Analysis (FPCA) via PACE. The principal component scores can
%            be 
...
%
%Input FVE_threshold:  a positive number that is between 0 and 1 [Default is 0.95.]
...
%                 if using automatic methods to choose K, i.e., 'AIC' or 'BIC'
%                 defined by selection_k.                [Default is 20.]
...
% Details:
%========
%       1)       bwmu = 0 and bwmu_gcv = 1 ==> use the geometric mean of the
%                minimum bandwidth and the GCV bandwidth as the bandwidth 
%                for estimating the mean function.
%
%                For Gaussian kernel, the optimal bandwidth from GCV is multiplied
%                by 1.1.
%
%                bwmu = 0 and bwmu_gcv = 0 ==> use CV method to choose the
%                bandwidth for the mean function.
%
%                bwmu = 0 and bwmu_gcv = 2 ==> use the GCV method to choose the 
%                bandwidth for the mean function.
%
...
%                of eigenvalues.
%
%       3)       If some arguments are not needed or you want to use
...
%
%       4)       In general, any output in the form of 1-dimensional
...
if isnumeric(selection_k)
  ...
  elseif selection_k <= 0
     fprintf(1,'Warning: selection_k must be a positive integer! Reset it to BIC now\n');
     selection_k = 'BIC';
     FVE_threshold = 0.95;
  end
end
...
if ~isempty(xcov)&&(length(xcov)==ngrid^2)
    ...
    if bwxcov(1)==0 || bwxcov(2)==0
        if (bwxcov_gcv == 1) || (bwxcov_gcv == 2)
            ...
            [bw_xcov] = cv_mullwlsn(y,t,mu,ntest1,ngrid1,regular,error,kernel,rcov,verbose);
        end
    elseif bwxcov > 0
        bw_xcov = bwxcov;
...
if ischar(selection_k)
    ...
    if k_id == 1
      [no_opt,AIC]=no_AIC(y,t,mu,bw_xcov,ngrid,regular,maxk,out1, out21,kernel, error,cut, rcov,xcov);
    elseif k_id == 2
      [no_opt,BIC]=no_BIC(y,t,mu, bw_xcov, ngrid,regular,maxk,out1, out21,kernel, error,cut, rcov,xcov);
    elseif k_id == 5
       no_opt = ngrid;
    end
    if isempty(no_opt)
...
if error==1
    [invalid, sigma]=pc_covE(t,out1,bw_xcov,ngrid,cut,kernel,rcov);
    if invalid==0
...
if strcmp(method_mu,'RARE') == 1    % this part modified by Wenwen       
    ...
    if regular == 1
     ...
     if ischar(selection_k)
         ...
         if k_id == 1
           [no_opt,AIC]=no_AIC(ynew,tnew,mu,bw_xcov,ngrid,regular,maxk, out1, out21,kernel, error,cut, rcov,xcov);  % y, t=>ynew, tnew
          elseif k_id == 2
           [no_opt,BIC]=no_BIC(ynew,tnew,mu, bw_xcov, ngrid,regular,maxk, out1, out21,kernel, error,cut, rcov,xcov);     % y, t=>ynew, tnew
          elseif k_id == 5
            no_opt = ngrid;
         end
...
% newxcov = reshape(interp2(out21copy, out21copy, xcov, xxi, yyi, 'spline'),length(out21),length(out21));
   %xcov = reshape(interp2(out21copy,out21copy,xcov,XI,YI,'spline'),length(out21),length(out21));
   xcov = reshape(griddata(out21copy,out21copy,xcov, XI,YI),length(out21), length(out21)); %added by Chris
        
%xcovfit = reshape(interp2(out21copy, out21copy, xcovfit, xxi, yyi, 'spline'),length(out21),length(out21));
  %xcovfit = reshape(interp2(out21copy,out21copy,xcovfit,XI,YI,'spline'),length(out21),length(out21));
xcovfit = reshape(griddata(out21copy, out21copy, xcovfit, XI,  YI),length(out21), length(out21)); %added by chris
   out1copy = out1;
   out1 = newdata;
end
%%---------------------------------------------------------------------------------------------------------------
...
%xcorr = max(min(xcorr,1),-1);
        xi_est_copy=xi_est;   % added by Wenwen, xi_var is a cell;
		xi_est=NaN(ncohort,size(xi_est,2));
        xi_est(yLength~=0,:)=xi_est_copy;
        if ~isempty(xi_var)
          xi_var_copy=xi_var;
          xi_var=cell(1,ncohort); 
          xi_var(1,yLength~=0)=xi_var_copy;    % added by Wenwen
        end
if corrPlot == 1
    if no_opt == 1
